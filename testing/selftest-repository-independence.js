...
const boundedPythonRecipeEvidenceObserverContract = JSON.parse(fs.readFileSync(path.join(ROOT, 'software-directions', 'placement', 'bounded-python-recipe-evidence-observer-hand.contract.json'), 'utf8'));
const boundedPythonRecipeReplayIsolationContract = JSON.parse(fs.readFileSync(path.join(ROOT, 'software-directions', 'placement', 'bounded-python-recipe-replay-isolation-hand.contract.json'), 'utf8'));
const boundedPythonRecipeAdmissionContract = JSON.parse(fs.readFileSync(path.join(ROOT, 'software-directions', 'placement', 'bounded-python-recipe-admission-plane.contract.json'), 'utf8'));
...
assert.strictEqual(boundedPythonRecipeEvidenceObserverContract.schema, 'axm.code.bounded-python-recipe-evidence-observer-hand-contract.v1');
assert.strictEqual(boundedPythonRecipeEvidenceObserverContract.authority, 'EXACT_SIX_FILES_READ_ONLY_NO_EXECUTION');
assert.strictEqual(boundedPythonRecipeEvidenceObserverContract.limits.requiredFileCount, 6);
assert.strictEqual(boundedPythonRecipeEvidenceObserverContract.truth.filePresenceAndBytesDirectlyObserved, true);
assert.strictEqual(boundedPythonRecipeEvidenceObserverContract.truth.javascriptParsedWithoutExecution, true);
assert.strictEqual(boundedPythonRecipeEvidenceObserverContract.truth.callerTestClaimsReproduced, false);
assert.strictEqual(boundedPythonRecipeEvidenceObserverContract.truth.observationIsMutationAuthority, false);
assert.strictEqual(boundedPythonRecipeEvidenceObserverContract.truth.observationIsPromotionAuthority, false);
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.schema, 'axm.code.bounded-python-recipe-replay-isolation-hand-contract.v1');
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.authority, 'FIXED_ISOLATION_POLICY_PROBE_ONLY_NO_PROPOSAL_EXECUTION');
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.limits.requiredIsolationProvider, 'bubblewrap');
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.limits.requiredResourceLimitProvider, 'prlimit');
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.truth.fixedTrustedPolicyProbeMayExecute, true);
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.truth.proposedSourceBytesRead, false);
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.truth.proposedBytesMountedInSandbox, false);
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.truth.callerTestClaimsReproduced, false);
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.truth.digestProvesSignerConsentOrIdentity, false);
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.truth.receiptProvesHandOrigin, false);
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.truth.nodePermissionModelTreatedAsMaliciousCodeSandbox, false);
assert.strictEqual(boundedPythonRecipeReplayIsolationContract.truth.nodeVmTreatedAsSecurityBoundary, false);
assert.strictEqual(boundedPythonRecipeAdmissionContract.schema, 'axm.code.bounded-python-recipe-admission-plane-contract.v1');
assert.strictEqual(boundedPythonRecipeAdmissionContract.version, '1.2.0');
assert.strictEqual(boundedPythonRecipeAdmissionContract.authority, 'STAGING_ONLY_NO_REGISTRY_OR_EXECUTION_AUTHORITY');
assert.strictEqual(boundedPythonRecipeAdmissionContract.limits.requiredEvidenceItems, 6);
assert.strictEqual(boundedPythonRecipeAdmissionContract.limits.requiredReadOnlyObservation, true);
assert.strictEqual(boundedPythonRecipeAdmissionContract.limits.requiredReplayIsolationAssessment, true);
assert.strictEqual(boundedPythonRecipeAdmissionContract.limits.activationGapCount, 6);
assert.strictEqual(boundedPythonRecipeAdmissionContract.truth.stagingMeansActiveAdmission, false);
assert.strictEqual(boundedPythonRecipeAdmissionContract.truth.callerEvidenceFilePresenceAndDigestsDirectlyObserved, true);
assert.strictEqual(boundedPythonRecipeAdmissionContract.truth.replayIsolationReceiptBound, true);
assert.strictEqual(boundedPythonRecipeAdmissionContract.truth.callerTestClaimsIndependentlyReproduced, false);
assert.strictEqual(boundedPythonRecipeAdmissionContract.truth.semanticSafetyIndependentlyVerified, false);
assert.strictEqual(boundedPythonRecipeAdmissionContract.truth.activeRegistryMutationAvailable, false);
assert.strictEqual(boundedPythonRecipeAdmissionContract.truth.activationAuthorizationAvailable, false);
assert.strictEqual(boundedPythonRecipeAdmissionContract.truth.automaticPromotionAvailable, false);
...
assert(readme.includes('placement/bounded-python-recipe-evidence-observer-hand.js'));
assert(readme.includes('placement/bounded-python-recipe-replay-isolation-hand.js'));
assert(readme.includes('placement/bounded-python-recipe-admission-plane.js'));
