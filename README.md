...
The expanded suite contains 26 checks: five generated-material drift checks and twenty-one deterministic selftests, including all 4,896 stable keyboard keys, all 29 software-direction profiles, 58 seed/stretch frontier reference builds, 58 deterministic placement plans, bounded read-only filesystem observations, two-target and six-target exact-byte commit/rollback trials, 53 actual `SIGKILL` crash/restart probes, concrete adapter receipts, adversarial placement/graph/editing/recovery holds, hybrid stacks, gap semantics, the standalone composition boundary, and repository independence. The committed verification transcript is under `testing/logs/`.
