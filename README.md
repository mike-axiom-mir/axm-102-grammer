...
The deterministic non-JavaScript application path is deliberately narrow. `software-directions/placement/bounded-python-recipe-registry.js` contains two exact entries with distinct builder and verifier-runner digests. The first uses `software-directions/placement/bounded-python-record-transform-author-hand.js` and `software-directions/placement/bounded-python-record-transform-verifier-adapter.js` to preserve the reviewed PR #51 donor route. The second uses `software-directions/placement/bounded-python-required-fields-author-hand.js` and `software-directions/placement/bounded-python-required-fields-verifier-adapter.js` to render and verify a bounded required-string-fields validator; it is independently implemented and current-source/adversarial-test bound, with no human-review claim. The host namespace sandbox is unavailable here, so both adapters refuse arbitrary Python and claim neither filesystem/network isolation nor general Python competence. A capable model remains the author outside these registered recipes.

`software-directions/placement/bounded-python-recipe-evidence-observer-hand.js` closes the proposal-file observation gap without receiving execution or mutation authority. It reads exactly six declared files beneath one absolute root, rejects traversal, symlinks, non-regular files, byte drift, and oversized inputs, recomputes their current digests, and parses two JavaScript sources plus four JSON artifacts without importing or executing them. Its five-minute receipt proves current file presence, bytes, digest matches, and parseability—not the caller's test claims, semantic safety, reviewer identity, consent, or human review.

`software-directions/placement/bounded-python-recipe-replay-isolation-hand.js` adds the prerequisite boundary for eventually reproducing caller test claims. It binds the proposal, evidence observation, environment observation, and one fixed empty-root Bubblewrap policy. On a capable host it executes only a trusted policy probe with user/PID/IPC/UTS/network namespaces, dropped capabilities, read-only runtime mounts, ephemeral `/tmp`, and `prlimit`; it never mounts or executes proposal bytes. On this host namespace creation is denied, so it emits a qualified, expiring hold without launching a policy-probe process. Node permissions and `node:vm` are deliberately not treated as malicious-code isolation.

`software-directions/placement/bounded-python-recipe-admission-plane.js` consumes both live receipts as the deliberately non-executing front door for possible future recipes. It validates one proposal's structure, binds the observed file digests and replay-isolation result, rejects active-entry and implementation collisions, and computes a candidate-entry plus hypothetical next-registry digest. Descriptor-only staging may retain a qualified host-isolation hold, but isolated adversarial replay is now a sixth explicit activation gap. The admission plane itself reads no workspace bytes, loads no proposed module, invokes no author or verifier, mutates no registry, issues no selection or authorization, and provides no promotion function.

`software-directions/placement/foundry-activation-plane.js` removes the handwritten assembly between those pieces. Given the manifest, one registry-bound recipe selection, live receipts, workspace/journal roots, and one explicit host activation authorization, it validates all six required Hand capsules, routes only the selected entry's author and verifier, authors the candidates, constructs the parser context, narrows host authority into the exact candidate-bound edit authorization, and invokes the durable writer/rollback path. Both registered recipes pass this route. The caller supplies no candidate bundle, parser context, or verifier adapter; the plane cannot issue authorization, cross-dispatch recipes, dynamically load code, or activate arbitrary model-authored Python.
...
The expanded suite contains 34 checks: five generated-material drift checks and twenty-nine deterministic selftests, including all 4,896 stable keyboard keys, all 29 software-direction profiles, 58 seed/stretch frontier reference builds, 58 deterministic placement plans, bounded read-only filesystem observations, exact-six-file proposal evidence observation, fail-closed replay-isolation qualification, environment-qualified Hand derivation, a two-entry bounded recipe registry, non-executing recipe staging, automatic activation of both registered recipes, spawned JavaScript/Python parser receipts, JavaScript and bounded-Python two-target commit/rollback trials, six-target JavaScript graph trials, 53 actual `SIGKILL` crash/restart probes, concrete adapter receipts, adversarial evidence/isolation/admission/registry/placement/foundry/activation/graph/editing/recovery holds, hybrid stacks, gap semantics, the standalone composition boundary, and repository independence. The committed verification transcript is under `testing/logs/`.
