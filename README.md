...
`software-directions/placement/hand-foundry-plane.js` now consumes the `requiredHands` emitted by one to four digest-bound placement plans and derives one capsule per unique role. More than one plan must also carry the validated edit-graph digest and installation order; a loose collection cannot be routed into the graph writer. It binds known implementations to the current language organ, grammar profile, targets, environment observation, and remaining authority gate. The first executable derivative is `software-directions/placement/spawned-parser-hand.js`: JavaScript candidates are parsed with Node's script grammar and Python candidates are passed as stdin to an isolated-mode `ast.parse` subprocess under fixed resource limits when `prlimit` is available. Neither parser executes candidate code. Existing JavaScript pair/graph writers and rollback paths are routed as authorization-required implementations; verifier execution remains host-binding-required. Missing authoring bodies, language writers, toolchains, or usable host sandboxing remain explicit capsule holds instead of fabricated Hands. The foundry itself reads no workspace, writes nothing, generates no code, and cannot authorize mutation or candidate execution.
...
The expanded suite contains 28 checks: five generated-material drift checks and twenty-three deterministic selftests, including all 4,896 stable keyboard keys, all 29 software-direction profiles, 58 seed/stretch frontier reference builds, 58 deterministic placement plans, bounded read-only filesystem observations, environment-qualified Hand derivation, spawned JavaScript/Python parser receipts, two-target and six-target exact-byte commit/rollback trials, 53 actual `SIGKILL` crash/restart probes, concrete adapter receipts, adversarial placement/foundry/graph/editing/recovery holds, hybrid stacks, gap semantics, the standalone composition boundary, and repository independence. The committed verification transcript is under `testing/logs/`.
