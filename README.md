...
`software-directions/placement/bounded-python-recipe-admission-plane.js` consumes that live read-only receipt as the deliberately non-executing front door for possible future recipes. It validates one proposal's structure, binds the observed file digests, rejects active-entry and implementation collisions, and computes a candidate-entry plus hypothetical next-registry digest. The receipt always remains staged and awaiting external review. The admission plane itself reads no workspace bytes, loads no proposed module, invokes no author or verifier, mutates no registry, issues no selection or authorization, and provides no promotion function.
...
The expanded suite contains 33 checks: five generated-material drift checks and twenty-eight deterministic selftests, including all 4,896 stable keyboard keys, all 29 software-direction profiles, 58 seed/stretch frontier reference builds, 58 deterministic placement plans, bounded read-only filesystem observations, exact-six-file proposal evidence observation, environment-qualified Hand derivation, a two-entry bounded recipe registry, non-executing recipe staging, automatic activation of both registered recipes, spawned JavaScript/Python parser receipts, JavaScript and bounded-Python two-target commit/rollback trials, six-target JavaScript graph trials, 53 actual `SIGKILL` crash/restart probes, concrete adapter receipts, adversarial evidence/admission/registry/placement/foundry/activation/graph/editing/recovery holds, hybrid stacks, gap semantics, the standalone composition boundary, and repository independence. The committed verification transcript is under `testing/logs/`.
