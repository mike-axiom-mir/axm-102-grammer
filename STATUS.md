...
- bounded Python author recipe present: record-field transform only
...
- automatic bounded Python Hand assemblies committed: 1
- caller-supplied candidate bundles/parser contexts/verifier adapters during activation: 0/0/0
- activation parser receipts: 4
- activation provenance-locked verifier passes: 1
- activation adversarial holds: 13
...
- The base organ selftest still reports general Python runtime correctness as `UNKNOWN`. The new runtime evidence is narrower: one exact donor recipe, regenerated and digest-bound to its author receipt, passed its emitted selftest and additional semantic probes.
