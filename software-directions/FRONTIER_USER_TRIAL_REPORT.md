...
The bounded graph extension then composed three dependency-linked placement entries into one six-target transaction without changing the original two-target API. Its focused tests produced one six-target commit, one intentional verifier failure with all six files and modes restored, 24 parser receipts, six verifier receipts, five application holds, and six planner holds. The graph recovery matrix killed workers at all 23 journal boundaries and verified 20 rollbacks, two recovered commits, and one already-committed finalization; five additional killed-worker scenarios covered restart replay, shared graph/pair lease exclusion, journal tampering, unknown bytes, mode drift, and a torn tail. Both editing Hands use the same workspace lease namespace.
...
This is still a narrow TEST capability. The stable pair API supports exactly two targets; the graph API supports two to four placement entries and four to eight targets. Both support Linux restart durability, JavaScript under Node's CommonJS script parse goal, existing parent directories, a durable journal, restart-surviving replay refusal, and an inter-process lease for cooperating Hand transactions. Neither claims universal power-loss safety, filesystem atomicity across its targets, automatic stale-lease breaking, proof of adapter purity, or protection from an external writer that bypasses the Hands. This is guidance and controlled application for code supplied elsewhere, not a claim of deterministic perfect code generation.
...
The separately authorized editing Hands now close the tested Linux process-crash/restart loop for both one JavaScript source/test pair and a bounded dependency graph of up to four pairs. The next placement maturation gap is real sandboxed language/parser/verifier adapters beyond Node's script goal, exercised against a production-like but disposable repository. Graphs above eight targets, conditional operations, deletes, moves, and broader filesystem guarantees remain later work. A later controlled trial should compare frontier and local models and measure whether the placement grammar actually reduces destination drift, repair work, and unrelated edits.
...
- separately authorized exact-byte pair editor, durable journal/recovery, transaction contract, and focused adversarial/crash tests: `placement/workspace-edit-hand.js`, `placement/workspace-edit-journal.js`, `placement/workspace-edit-hand.contract.json`, `placement/selftest-workspace-edit-hand.js`, `placement/selftest-workspace-edit-recovery.js`
- bounded dependency-graph planner and editor, graph journal/recovery, contracts, and focused adversarial/crash tests: `placement/edit-graph-plane.js`, `placement/workspace-edit-graph-hand.js`, `placement/workspace-edit-graph-journal.js`, `placement/edit-graph-plane.contract.json`, `placement/workspace-edit-graph-hand.contract.json`, `placement/selftest-edit-graph-plane.js`, `placement/selftest-workspace-edit-graph-hand.js`, `placement/selftest-workspace-edit-graph-recovery.js`
- deterministic 58-build verification: `selftest-frontier-direction-workbench.js`

Aggregate adapter-and-placement-bound frontier report digest: `348e83e2ddd7b3e4e0918a7df9a75cec099e4efbf4b40d91aa7088d60c205e8f`
