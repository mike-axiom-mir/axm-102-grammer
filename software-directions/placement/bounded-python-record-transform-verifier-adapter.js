...
const ADAPTER_ID = 'bounded-python-record-transform-unit-test';
...
const RUNNER = [
  ...
  'safe_builtins={"type":type,"dict":dict,"str":str,"len":len,"any":any,"TypeError":TypeError,"ValueError":ValueError,"OverflowError":OverflowError,"RecursionError":RecursionError,"__import__":candidate_import,"print":lambda *values,**kwargs:messages.append(" ".join(str(value) for value in values))}',
  ...
  'run=capability.run',
  'probe={config["sourceField"]:"semantic-probe"}',
  'first=run(probe)',
  'second=run(probe)',
  'assert first==second and first["ok"] is True',
  'assert first["output"][config["targetField"]]=="semantic-probe"',
  'assert run([])["code"]=="INPUT_OBJECT_REQUIRED"',
  'oversized={str(index):"x" for index in range(config["maxInputKeys"]+1)}',
  'assert run(oversized)["code"]=="INPUT_KEY_LIMIT"',
...
function environmentBinding(observation) {
  ...
  const python = environmentHand.get(observation, 'python3');
  const prlimit = environmentHand.get(observation, 'prlimit');
  if (!python?.usable) throw Error('BOUNDED_PYTHON_VERIFIER_PYTHON_UNAVAILABLE');
  if (!prlimit?.usable || observation.resourceLimits?.usable !== true) throw Error('BOUNDED_PYTHON_VERIFIER_RESOURCE_LIMITS_UNAVAILABLE');
  return {
    python: {toolId: python.id, executablePath: python.executablePath, executablePathSha256: python.executablePathSha256, version: python.version},
    prlimit: {toolId: prlimit.id, executablePath: prlimit.executablePath, executablePathSha256: prlimit.executablePathSha256, version: prlimit.version},
    resourceLimits: {...observation.resourceLimits}
  };
}

function expectedReceipt(receipt) {
  authorHand.validateReceipt(receipt);
  if (receipt.donorImplementationSha256 !== authorHand.donorImplementationSha256()) throw Error('BOUNDED_PYTHON_VERIFIER_DONOR_IMPLEMENTATION_DRIFT');
  const generated = authorHand.buildPythonRecordTransform(receipt.parameters);
  const expected = {
  ...
  if (registry.canon(expected) !== registry.canon(receipt.candidates)) throw Error('BOUNDED_PYTHON_VERIFIER_AUTHOR_RECEIPT_CANDIDATE_MISMATCH');
  return receipt;
}

function create({authorReceipt = null, environmentObservation = null} = {}) {
  const boundReceipt = expectedReceipt(authorReceipt);
  const binding = environmentBinding(environmentObservation);
  const metadata = {
    ...
    donorBuilderSha256: authorHand.DONOR.builderSha256, runnerSha256: RUNNER_SHA256,
    ...
    truth: {hostNamespaceSandbox: false, hostFilesystemIsolation: false, hostNetworkIsolation: false, arbitraryCandidateExecution: false, exactDonorCandidateRequired: true, candidateRuntimeCorrectnessUniversallyClaimed: false},
    authority: AUTHORITY
  };
  return Object.freeze({
    ...
    verify(context) {
      try {
        const currentBinding = environmentBinding(environmentObservation);
        if (registry.canon(currentBinding) !== registry.canon(binding)) return {passed: false, observations: {errorCode: 'ENVIRONMENT_BINDING_DRIFT'}};
      } catch (error) {
        return {passed: false, observations: {errorCode: 'ENVIRONMENT_BINDING_INVALID'}};
      }
      ...
      const outcome = spawnSync(binding.prlimit.executablePath, args, {
        input: payload, encoding: 'utf8', cwd: path.parse(process.cwd()).root,
        timeout: limits.wallTimeoutMs, maxBuffer: MAX_OUTPUT_BYTES,
        env: {PATH: '/usr/local/bin:/usr/bin:/bin', LANG: 'C', LC_ALL: 'C', PYTHONNOUSERSITE: '1', PYTHONDONTWRITEBYTECODE: '1'}
      });
      const stdout = String(outcome.stdout || ''); const stderr = String(outcome.stderr || '');
      let parsed = null;
      ...
      return {
        passed,
        observations: {
          result: passed ? 'PROVENANCE_LOCKED_RUNTIME_PASS' : 'PROVENANCE_LOCKED_RUNTIME_FAIL',
          errorCode: passed ? null : (outcome.error?.code === 'ETIMEDOUT' ? 'CANDIDATE_TIMEOUT' : 'CANDIDATE_PROCESS_OR_OUTPUT_FAILED'),
          processStatus: outcome.status, processSignal: outcome.signal || null,
          capturedSelftestPrintCount: parsed?.capturedPrintCount ?? null,
          stdoutSha256: sha256(stdout), stderrSha256: sha256(stderr),
          donorBuilderSha256: authorHand.DONOR.builderSha256, runnerSha256: RUNNER_SHA256,
          hostNamespaceSandbox: false, arbitraryCandidateExecution: false, exactDonorCandidateExecuted: true
        }
      };
    }
  });
