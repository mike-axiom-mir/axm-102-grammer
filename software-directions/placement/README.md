...
Other capsules truthfully route what exists: the read-only map Hand is available without editing authority; JavaScript and Python pair write/rollback Hands require separate host authorization; JavaScript verifiers require digest-bound host adapters. Python author and verifier capsules bind the same two-entry bounded-recipe registry digest and remain `RECIPE_SELECTION_REQUIRED` until exact entry, layout, parameters, and authorization evidence are supplied. Each provenance-locked verifier requires its own author receipt and cannot accept the other entry's receipt. An installed `bwrap` binary is not called usable when its harmless namespace probe fails. The foundry cannot generate missing code, grant mutation, grant candidate execution, install tools, deploy, promote, or change canon.
...
## Bounded Python recipe registry

`bounded-python-recipe-registry.js` replaces activation's hard-coded author/verifier imports with a sorted, digest-bound registry. Its two entries independently bind recipe identity, builder identity and digest, author receipt schema/result, verifier identity and runner digest, provenance class, and explicit refusal of general authoring or arbitrary execution:

- `bounded-python-record-transform` retains the externally reviewed PR #51 donor binding;
- `bounded-python-required-fields` generates a dependency-free validator for one bounded list of required non-empty string fields. It has a separate builder, receipt schema, verifier runner, and adversarial suite. It is current-source/test bound and makes no human-review claim.
...
## Read-only proposal evidence observer

`bounded-python-recipe-evidence-observer-hand.js` reads exactly six explicitly declared proposal files beneath one absolute workspace root: author source/contract, verifier source/contract, parameter contract, and adversarial-test receipt. It refuses broad or relative roots, traversal, aliases, symlinks in any path component, non-regular files, invalid UTF-8, byte drift, digest mismatches, and inputs above one MiB per file or six MiB total. The Hand hashes current bytes and syntax-parses the two CommonJS JavaScript sources plus four JSON objects without importing modules or executing candidate code.

Its five-minute observation binds the proposal, evidence envelope, declaration, root identity, exact paths, current file bytes, parse results, and three core proposal digests. This directly verifies file presence, bytes, digest matches, and parseability. It does not reproduce the caller's adversarial tests, analyze source semantics, establish reviewer identity or consent, complete human review, or grant mutation, activation, or promotion authority. The focused trial observes all six files, proves malicious top-level fixture code never runs, performs zero imports, child processes, writes, executions, registry changes, or promotions, and verifies nineteen adversarial holds.

## Replay-isolation qualification

`bounded-python-recipe-replay-isolation-hand.js` binds the proposal and both live observations to a single hashed sandbox policy. It does not accept a caller command, module path, source buffer, test runner, mount, or environment. When Bubblewrap, `prlimit`, and Node are usable, the only process it may start is a fixed trusted policy probe inside an empty-root sandbox: user, PID, IPC, UTS, network, and cgroup namespaces; UID/GID 65534; all capabilities dropped; read-only `/usr` and one exact Node binary; ephemeral `/tmp`; no proposal or workspace mount; cleared environment; and CPU, address-space, descriptor, process, output-file, core, and wall-time limits.

The probe must directly observe hidden host paths, read-only runtime mounts, ephemeral writes, a bounded PID namespace, non-root identity, fixed hostname, and blocked networking. A failed assertion produces a qualified policy hold, never certification. On the current verification host Bubblewrap is installed but namespace creation is denied, so the Hand emits `RECIPE_REPLAY_ISOLATION_HELD_HOST_PROVIDER_UNAVAILABLE` and starts zero additional processes. Sixteen adversarial cases cover input/digest/observation/environment/time/binding/receipt/policy/authority drift. Even certification would not execute proposal code, reproduce caller claims, establish semantic safety, grant replay authority, or activate a recipe.

## Non-executing recipe admission and staging

`bounded-python-recipe-admission-plane.js` structurally checks one proposed future registry entry without importing its implementation. The proposal cannot contain a module path and must explicitly refuse general Python authoring, arbitrary execution, and dynamic loading. The caller evidence envelope still carries test claims, but staging now requires both a live observer receipt proving the exact six current files and digests were read and parsed without import and a live qualified replay-isolation receipt bound to that observation.

The admission plane does not itself read workspace bytes, reproduce the tests, infer semantic safety, or claim human review. A qualified host-unavailable or policy-probe hold may enter descriptor-only staging, but the receipt retains six unresolved activation gaps: isolated adversarial replay, human review, explicit registry source change, full regression, fresh Foundry manifest, and fresh host authorization. It cannot make the entry active, select it, execute it, authorize it, promote it, or change canon; no promotion API exists.

The focused trial produces the same staging receipt twice, leaves the active two-entry registry byte-identical, and confirms the staged recipe remains unselectable. Zero proposed module loads, author/verifier calls, candidates, admission child processes, registry mutations, selections, authorizations, or promotions occur. Twenty-four adversarial holds cover missing/forged registries, proposal/evidence digest drift, unsafe identifiers or authority, active and implementation collisions, incomplete/duplicated evidence, unsafe test claims, forged review state, attempted module-path injection, missing/tampered/stale/future/mismatched observer receipts, and missing/tampered/mismatched/stale replay-isolation receipts.
...
The recipe selection binds the registry, entry, recipe, builder, verifier runner, and parameters. The separate `EXPLICIT_SINGLE_ACTIVATION` authorization binds that selection plus the manifest, placement plan, project/environment receipts, workspace and journal identities, author/parser/verifier capsules, selected implementations, TTL, rollback, and durable recovery. The plane validates that authority and deterministically narrows it into the existing exact candidate-bound edit authorization; it does not create or broaden permission.

The focused activation trial supplies zero candidate bundles, zero parser contexts, and zero verifier adapters from the caller. It automatically assembles the author, parser, recipe-specific verifier, writer, and required rollback capsule for both entries, commits two two-target Python transactions, produces eight parser receipts and two provenance-locked runtime passes, and leaves unrelated human files unchanged. Sixteen holds cover repeat activation after state change, missing authority, manifest or registry-capsule tamper, missing rollback implementation, selection/registry/cross-recipe tamper, unsupported recipe, invalid parameters, authorization binding/authority/staleness, parser mismatch, environment tamper, and workspace drift. All hold fixtures remain byte-identical at return.

This remains a bounded registry of two recipes, one Python pair per explicit activation, and no dynamic plugin loading. It is not a general recipe system or deterministic Python coder. There is no arbitrary candidate activation, graph activation, self-authorization, deployment, promotion, or canon authority. Durable rollback, recovery, replay refusal, and lease behavior come from the already-tested pair Hand rather than being reimplemented.
